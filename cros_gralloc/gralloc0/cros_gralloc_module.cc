			      ...
			      buffer_handle_t *handle, int *stride)
{
	...
	descriptor.drv_usage = cros_gralloc_convert_flags(usage);
			     ...
			     int usage, int l, int t, int w, int h, void **vaddr)
{
	...
	fence = -1;
	flags = cros_gralloc_convert_flags(usage);
	ret = mod->driver->map(handle, fence, flags, addr);
	*vaddr = addr[0];
	return ret;
}
...
static int cros_gralloc_unlock(struct gralloc_module_t const *module, buffer_handle_t handle)
{
	int32_t fence_fd;
	auto mod = (struct cros_gralloc_module *)module;
	return mod->driver->unmap(handle, &fence_fd);
}
